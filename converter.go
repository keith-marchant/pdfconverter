...
// aws lambda create-function --function-name go-html-pdf-converter --runtime provided.al2 --handler bootstrap --architectures arm64 --role test-stream-function-role --zip-file fileb://converter.zip
...
func GetPagePdf(url string) (data []byte, err error) {
	...
	err = rod.Try(func() {
		...
		browserLauncher := launchInLambda()
		...
		// return the html
		r, err := page.Timeout(htmlTimeout).PDF(&proto.PagePrintToPDF{
			Landscape:           false,
...
func Convert(url string) ([]byte, error) {
...
func SaveFile(data []byte, key string) (bool, error) {
	sess := session.Must(session.NewSession())
	reader := bytes.NewReader(data)
	svc := s3.New(sess)
	input := &s3.PutObjectInput{
		Body:   aws.ReadSeekCloser(reader),
		Bucket: aws.String("kmarchant-document-manager-test"),
		Key:    aws.String(key),
	}
